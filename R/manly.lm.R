#' Manly transformation for linear and linear mixed models
#'
#' Depending on the class of the first object, this function estimates the 
#' optimal transformation parameter for the Manly transformation for 
...
manly.lm <- function(object, lambda, method, lambdarange = c(-2, 2),
  ...
  if (lambda == "estim") {
    bx_cxOptim <- est_lm(y = y, x = x, transfor = "t_mnl", method = method, 
  ...
  if (plotit == TRUE) {
    ...
    measvector <- sapply(lambdavector, estim_lm, y = y, x = x, transfor = "t_mnl",
                         method = method)
