#' Bickel-Doksum transformation for linear and linear mixed models
#'
#' Depending on the class of the first object, this function estimates the 
#' optimal transformation parameter for the Bickel-Doksum transformation for 
...
#' @keywords internal
#' @export
bickeldoksum.lme <- function(object, lambda, method, lambdarange = c(1e-11, 2),
                             plotit = TRUE, ...) {
  ...
  if (lambda == "estim") {
    bickeldoksumOptim <- est_lme(y = y, x = x, formula = formula, data = data, 
                                 rand_eff = rand_eff, method = method, 
                                 lambdarange = lambdarange, transfor = "t_bck_dk") 
